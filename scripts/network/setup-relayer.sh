...
# quick commands
# ./build/binary/junod tx ibc-transfer transfer transfer channel-0 osmo15hh4c5dzwdy6alx6uzc5c2hzd3eu2dn2588m70 1000000000ujuno --from test1 --keyring-backend test --home scripts/network/config/juno --chain-id test-juno --fees 100000ujuno --yes --node tcp://localhost:26657
# ./build/binary/osmosisd tx gamm create-pool --pool-file scripts/network/juno-osmosis-pool.json --from test1 --keyring-backend test --home scripts/network/config/osmosis --chain-id test-osmo --fees 100000uosmo --yes --node tcp://localhost:26357